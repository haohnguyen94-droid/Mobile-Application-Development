...
class DBHelper{
  ...
  Future<Database> _initDb() async{
    ...
    */
    ...
    version: 2 means this is version 2 of your database structure
    (version 1 had no 'date' column).
    onCreate: _onCreate means:
    When the database is created for the first time, run the _onCreate function.
    that function will create your tables.
    onUpgrade: _onUpgrade runs when an OLD database (still on version 1) is opened,
    so existing users keep their notes and just gain the new 'date' column. */
  ...
  Future<void> _onCreate(Database db, int version) async{
    ...
    */
    await db.execute( 
      ...
      'date TEXT)',//creates a date column storing the creation date as text.
    );
  }

  /*This function runs when an existing database is opened with a higher version
  number than it was created with. Here it migrates a version 1 database
  (which had no 'date' column) up to version 2 by adding that column.
  Existing notes are kept; their new 'date' value starts as NULL and is
  handled by Note.fromMap.
  */
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async{
    if(oldVersion < 2){
      // ALTER TABLE adds the new column to the already-existing notes table.
